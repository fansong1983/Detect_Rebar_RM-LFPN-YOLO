...
class YOLOLoss(nn.Module):
    ...
    def box_ciou(self, b1, b2):
        ...
        # ciou    = ciou - alpha * v
        # ----------加入对长宽的惩罚项 EIOU--------- #
        w_distance = torch.pow((b1_wh[..., 0] - b2_wh[..., 0]), 2) / torch.clamp(torch.pow(enclose_wh[..., 0],2),min = 1e-6)
        h_distance = torch.pow((b1_wh[..., 1] - b2_wh[..., 1]), 2) / torch.clamp(torch.pow(enclose_wh[..., 1],2),min = 1e-6)
        # v = 1.0 * torch.pow(iou, 0.5)
        ciou    = ciou - w_distance - h_distance
        # -------------------改的------------------- #
